...
### VERDICT: content validation PASSED (2026-07-19)

Run 3 (survived a battery-death hibernation mid-run, then hit a second silent external kill at ~83%) added **zero new keys** — its 56 shards are a strict subset of run 2's 75. Decisively, the validated 75 already include **every large-tile class**: all six `/128d` tiles (`/1h`…`/1d`), `/30m@64d`, `/3d@96d`, `/7d@896d` — the epoch-aligned deep tiles end 2026-06-04 and closed mid-run. Residual gap = only trailing tiles covering ~[07-06, 07-18) per tier: no new code path (validated siblings at the same rungs; clip logic covered by the genesis-clipped head tiles), and exact-compare on trailing partials is ill-defined anyway (the fan-out built them with a later data cutoff than the engine's TO).

**75/75 EQUAL, 0 diff, spanning all 15 tiers × all rung classes, incl. planner agreement (identical min-cover key sets). The engine's output is content-identical to the independent Lambda materializer's.** The pyrmts build-engine spec's validation gate is met; the clean end-to-end wall number + amd64/cross-arch determinism check ride with the first Batch `submit --watch` smoke (this host demonstrably can't hold a 3-hour process: sleeps, battery, two unexplained SIGKILLs).

Additional finding for pyrmts: over a multi-hour run the process **footprint** grows to ~55 GB (Activity Monitor; ~23 GB system swap) even while RSS stays ~5 GB — polars/allocator retention of freed combine memory, paged out and never touched again. Harmless for one-shot containers, but an allocator trim (e.g. jemalloc purge / `malloc_trim` equivalent) after each shard close would keep long runs polite on shared hosts.

Follow-through in ctbk (committed): pinned-source cutover to `7fad0f5` (`c167728f`), venv synced off the editable link, and the `ExpectedShard.effective_{start,end}` compat fix in `decode_gap`/scaffold slots + tests (`14a1b88f`, latent since `5409dfc` — would have broken the next Lambda deploy). Scratch prefix `avail-v4-engine-check/` retained for the Batch smoke's compare; delete after.
