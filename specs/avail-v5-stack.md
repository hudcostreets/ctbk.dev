...
- **Promotion should be CoW via the registry** (2026-07-28 discussion): the inventory-driven read walk fetches by the D1 row's `key` verbatim (`planner.ts` `keys.push(chosen.key)`), so a new pyramid can be stood up as pure D1 rows pointing at existing objects — `INSERT … SELECT` with the pyramid renamed, keys kept, zero copies. Gated on: (a) gap discovery going registry-driven (today it lists R2 under the keyTemplate prefix, so CoW history looks "missing" and the Lambda would rebuild it), (b) namespace-aware GC (a sweeper must not delete objects another pyramid's rows reference). Interim `engine promote` (server-side copy + re-keyed manifest) remains the copy-based fallback. (The v5 backfill re-ran the proven job instead; harmless — byte-identical to `manifest-batch2`, 99/99 md5s — but promotion would have been free.)
- **Unifying frame** (steady-state doctrine: per-tier cron writers + one fsck-style backfill, no third shapes): one declarative contract — actual coverage ≡ expected min-cover of `[genesis, now]` — two executors: cascade Lambda (incremental) + Batch engine (bulk). Engine end-state: diff expected-vs-registry and build only the missing set (fsck mode), rather than rebuilding an explicit range against a private JSONL manifest; with same-tier consolidation, "backfill" / "resume" / "extend" / "promote" collapse into that one operation.
