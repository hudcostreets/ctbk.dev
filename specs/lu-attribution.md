...
Status: **avail-v6 default (cutover 2026-08-10)** (built 2026-08-06; poller v2 2026-08-03). Burn-in exit checks (2026-08-10): recent-window (08-08→08-09) v5/v6 responses byte-equal; old-window (06-01→06-08) deltas are the intended re-attribution correction (215/336 bins shift slightly, e.g. mean 5.586→5.611); tip freshness equal to v5 (same last bin). `DEFAULT_PYRAMID = 'avail-v6'`; v5 stays addressable behind `?pyramid=avail-v5` until GC (pending sign-off). The full-history LU-attributed regen ran as one Batch job (`DailyStatusSource` raw-ingest, 5.35B source rows → 187 shards / 29GB, 73 min): genesis→2026-08-04 watermark by the engine, tip by the new `ctbk-avail-cascade-v6-tick` (same Lambda, config-driven). Era-seam verification: post-cutover windows byte-equal to v5; pre-cutover row-overlap peaks at +1min shift (drift as measured), and count mass = exactly (distinct-LU minutes)/(poll minutes) — v6 removes the ~3% duplicate-LU double-counting the old poller baked into v5 history. Addressable via `?pyramid=avail-v6`; remaining: burn-in → default flip → v5 GC. Burn-in day 1 caught a serving gap: the api worker's registry-reconcile cron (sole registrar for tick-written shards) only covered `avail-v5`, so v6's tip shards sat on R2 unregistered and `?pyramid=avail-v6` served nothing after the Batch watermark — fixed by adding v6 to `RECONCILE_PYRAMIDS`/`HEALTH_PYRAMIDS` (+ `defaultPyramid` in the /health snapshot; FE collapses non-default pyramids). Decision: the feed's `last_updated` (LU) is the correct time-of-record for availability snapshots; the poller's wall clock (`polled_at`) is operational metadata. Requires a poller rewrite (cheap, immediate win) and a full pyramid regen (bundled with drop-LUC — see Sequencing). Companion: `specs/drop-luc-station-keys.md`.
...
4. **The 2.3 URL initially probed at ~3–5s detection latency — but that was cache-phase luck, not a stable property.** (CORRECTION, 2026-08-04: a laptop re-probe a day later showed ~42s on 2.3, and the deployed worker sees ~40–65s from its colo. Both endpoints sit behind free-running ~60s CloudFront caches; detection latency = (edge-expiry phase − origin-regen phase) mod 60, wandering slowly over hours. There is NO request-side freshness lever: `Cache-Control: no-cache` is ignored and query-string busting is ignored — random `?cb=` still returns `x-cache: Hit` with nonzero `Age`.) 2.3 is still the right endpoint: current spec version (2022-04-05 release; Lyft's `gbfs_versions.json` publishes exactly {v1.1, v2.3} — 1.1 is the legacy 2019-era endpoint our poller inherited), and its station fields are a strict superset (adds `vehicle_types_available`).
5. **Day-1 capture audit (2026-08-04, first ~22h of poller v2)**: 1326 WAL keys, 5 empty calendar minutes, **zero capture failures** — 4 were upstream skipped generations (LU deltas 120–125s across the hole; no consumer could see those minutes) and 1 was cadence jitter straddling a bin (LUs at :58 and next-minute :00 — both captured, the middle minute simply has no LU). Upstream skips ~4/day is the observed norm; miss-monitors must classify by LU delta (>110s across a hole = upstream skip, don't page), not by calendar-key presence.
6. **LU is NOT minute-aligned**: observed `lu % 60` ∈ {5, 6, 7} (origin stamps ~:05 past the minute), with ±1–2s cadence jitter (deltas 58–62s). Binning by `floor(ts/60)` is a real assignment, not a formality, and LU phase can drift across minute boundaries over time.
...
- **Drift TS expectation (post-correction)**: `polled_at − ts` wanders 0–60s with the cache phase (long-run mean ~30s), vs the old regime's compounded cache-phase + cron-phase (~93s observed). The structural win is capture-completeness + exact attribution, not sub-10s freshness — the CDN's ~60s TTL is a hard floor we cannot bust. Alert if drift sustains > ~90s (beyond one full cache cycle + jitter — indicates a second cache layer or endpoint regression).
