...
def resolve_cross_dump_dups(df: DataFrame, name: str) -> DataFrame:
    ...
    """
    # Mirror `dedupe_sort`'s uniqueness key exactly: Ride ID (2020+) else Bike ID
    # (pre-2020, when Ride ID doesn't exist). Keying on `(Start, Stop)` alone
    # would collapse distinct rides that merely share a timestamp pair.
